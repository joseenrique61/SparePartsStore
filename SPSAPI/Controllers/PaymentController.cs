...
public class PaymentController(IVaultKmsService kmsService, ApplicationDBContext context, IConfiguration configuration, ILogger<PaymentController> logger) : ControllerBase
{
    [HttpPost("process")]
    ...
    public async Task<IActionResult> GenerateRedirectUrl([FromBody] OrderRequest orderRequest)
    {
        ...
        var payload = new
        {
            ...
            redirectUrl = configuration.GetSection("MVC:Url").Value,
        ...
        string systemBUrl = $"{configuration.GetSection("SystemB:Url").Value}/checkout?token={encodedToken}";
